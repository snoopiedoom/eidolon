...
static void capture_runtime_settings(const EidolonApp *app, EidolonUserSettings *settings) {
    ...
    settings->presentation_preference = (int)app->presentation_preference;
...
static void mark_user_settings_dirty(EidolonApp *app, EidolonUserSettingField field) {
    ...
    case EIDOLON_USER_SETTING_RENDER_MODE:
        ...
        break;
    case EIDOLON_USER_SETTING_PRESENTATION:
        app->user_settings.presentation_preference = (int)app->presentation_preference;
        break;
...
static bool ensure_sprite(EidolonApp *app) {
    if (eidolon_sprite_ready(app->sprite)) {
        return eidolon_sprite_set_renderer(app->sprite, app->renderer);
    }
    char path[1024];
...
static const char *character_config_path(void) {
#if !defined(NDEBUG)
    const char *test_path = SDL_getenv("EIDOLON_PRESENTATION_TEST_CHARACTER_CONFIG");
    if (test_path != NULL && test_path[0] != '\0') {
        return test_path;
    }
#endif
    return EIDOLON_CHARACTER_CONFIG_PATH;
}
...
static bool ensure_portrait(EidolonApp *app) {
    ...
    app->portrait =
        eidolon_portrait_create(app->renderer, character_config_path(), EIDOLON_ASSET_DIR);
...
static bool ensure_model(EidolonApp *app) {
    ...
    if (model_path == NULL || model_path[0] == '\0') {
        ...
        eidolon_log_write("model", "using compiled default 3D body path=%s", model_path);
    } else {
...
static bool ensure_body_renderer(EidolonApp *app, EidolonRenderMode mode) {
    switch (mode) {
    case EIDOLON_RENDER_MODE_SPRITE:
        return ensure_sprite(app);
    case EIDOLON_RENDER_MODE_PORTRAIT:
        return ensure_portrait(app);
    case EIDOLON_RENDER_MODE_MODEL_3D:
        return ensure_model(app);
    case EIDOLON_RENDER_MODE_COUNT:
        break;
    }
    SDL_SetError("invalid body renderer");
    return false;
}

static bool body_renderer_supports_native_targets(EidolonRenderMode mode) {
    switch (mode) {
    case EIDOLON_RENDER_MODE_SPRITE:
    case EIDOLON_RENDER_MODE_PORTRAIT:
    case EIDOLON_RENDER_MODE_MODEL_3D:
...
static bool activate_initial_body_renderer(EidolonApp *app, EidolonRenderMode requested) {
    static const EidolonRenderMode fallbacks[] = {
        EIDOLON_RENDER_MODE_PORTRAIT,
        EIDOLON_RENDER_MODE_SPRITE,
        EIDOLON_RENDER_MODE_MODEL_3D,
    };
    if (ensure_body_renderer(app, requested)) {
        app->render_mode = requested;
        return true;
    }
    char requested_error[256];
    SDL_strlcpy(requested_error, SDL_GetError(), sizeof(requested_error));
    SDL_ClearError();
    for (size_t index = 0U; index < SDL_arraysize(fallbacks); ++index) {
        const EidolonRenderMode fallback = fallbacks[index];
    ...
    SDL_SetError("body renderer %s failed (%s), and no fallback renderer is available",
                 eidolon_render_mode_name(requested), requested_error);
    return false;
}
...
static bool presentation_output_was_removed(EidolonApp *app,
                                            const EidolonPresentationEnvironment *previous,
                                            const EidolonPresentationEnvironment *current) {
    if (previous == NULL || current == NULL ||
        (previous->valid_fields & EIDOLON_PRESENTATION_ENV_ACTIVE_OUTPUT) == 0U ||
        previous->active_output.value == 0U ||
        (current->valid_fields & EIDOLON_PRESENTATION_ENV_OUTPUT_TOPOLOGY) == 0U ||
        current->topology_revision == previous->topology_revision) {
        return false;
    }
    ...
    for (size_t index = 0U; index < output_count; ++index) {
        if (outputs[index].output.value == previous->active_output.value) {
            removed = false;
            break;
        }
    }
...
static bool map_removed_output_body_center(const EidolonPresentationEnvironment *previous,
                                           const EidolonPresentationEnvironment *current,
                                           SDL_FPoint *center) {
    if (previous == NULL || current == NULL || center == NULL ||
        previous->coordinate_space != current->coordinate_space ||
        (previous->valid_fields & EIDOLON_PRESENTATION_ENV_USABLE_BOUNDS) == 0U ||
        (current->valid_fields & EIDOLON_PRESENTATION_ENV_USABLE_BOUNDS) == 0U ||
        previous->usable_bounds.width <= 0.0F || previous->usable_bounds.height <= 0.0F ||
        current->usable_bounds.width <= 0.0F || current->usable_bounds.height <= 0.0F) {
        return false;
    }

    const bool logical_scale_valid =
        (previous->valid_fields & EIDOLON_PRESENTATION_ENV_CONTENT_SCALE) != 0U &&
        (current->valid_fields & EIDOLON_PRESENTATION_ENV_CONTENT_SCALE) != 0U &&
        previous->content_scale > 0.0F && current->content_scale > 0.0F;
    if (logical_scale_valid) {
        const float local_x = (center->x - previous->usable_bounds.x) / previous->content_scale;
        const float local_y = (center->y - previous->usable_bounds.y) / previous->content_scale;
        center->x = current->usable_bounds.x + local_x * current->content_scale;
        center->y = current->usable_bounds.y + local_y * current->content_scale;
    } else {
        const float normalized_x =
            (center->x - previous->usable_bounds.x) / previous->usable_bounds.width;
        const float normalized_y =
            (center->y - previous->usable_bounds.y) / previous->usable_bounds.height;
        center->x = current->usable_bounds.x + normalized_x * current->usable_bounds.width;
        center->y = current->usable_bounds.y + normalized_y * current->usable_bounds.height;
    }
    return true;
}

static void clamp_body_center_to_environment(const EidolonApp *app,
                                             const EidolonPresentationEnvironment *environment,
                                             SDL_FPoint *center) {
    if (app == NULL || environment == NULL || center == NULL ||
        (environment->valid_fields & EIDOLON_PRESENTATION_ENV_USABLE_BOUNDS) == 0U ||
        environment->usable_bounds.width <= 0.0F || environment->usable_bounds.height <= 0.0F) {
        return;
    }
    float body_width = 0.0F;
    float body_height = 0.0F;
    body_dimensions(app, app->model_scale, &body_width, &body_height);
    const float half_width = body_width * app->window_coordinate_scale * 0.5F;
    const float half_height = body_height * app->window_coordinate_scale * 0.5F;
    const float center_x = environment->usable_bounds.x + environment->usable_bounds.width * 0.5F;
    const float center_y = environment->usable_bounds.y + environment->usable_bounds.height * 0.5F;
    const float minimum_x = environment->usable_bounds.x + half_width;
    const float maximum_x =
        environment->usable_bounds.x + environment->usable_bounds.width - half_width;
    const float minimum_y = environment->usable_bounds.y + half_height;
    const float maximum_y =
        environment->usable_bounds.y + environment->usable_bounds.height - half_height;
    center->x = minimum_x <= maximum_x ? SDL_clamp(center->x, minimum_x, maximum_x) : center_x;
    center->y = minimum_y <= maximum_y ? SDL_clamp(center->y, minimum_y, maximum_y) : center_y;
}
...
static void apply_settings_layer(EidolonApp *app, const EidolonUserSettings *settings) {
    ...
    if (eidolon_user_settings_is_overridden(settings, EIDOLON_USER_SETTING_PRESENTATION) &&
        settings->presentation_preference >= 0 &&
        settings->presentation_preference < (int)EIDOLON_PRESENTATION_PREFERENCE_COUNT) {
        eidolon_app_set_presentation_preference(
            app, (EidolonPresentationPreference)settings->presentation_preference);
    }
    if (eidolon_user_settings_is_overridden(settings, EIDOLON_USER_SETTING_RENDER_MODE) &&
        ...
        const EidolonRenderMode requested = (EidolonRenderMode)settings->render_mode;
        if (!eidolon_app_set_render_mode(app, requested)) {
            eidolon_log_write("settings", "renderer unavailable requested=%s active=%s reason=%s",
                              eidolon_render_mode_name(requested),
                              eidolon_render_mode_name(app->render_mode), SDL_GetError());
            SDL_ClearError();
        }
    }
...
static bool presentation_test_ignore_user_settings(void) {
#if !defined(NDEBUG)
    const char *ignore_settings = SDL_getenv("EIDOLON_PRESENTATION_TEST_IGNORE_USER_SETTINGS");
    return ignore_settings != NULL && SDL_strcmp(ignore_settings, "1") == 0;
#else
    return false;
#endif
}

static bool body_renderer_test_override(EidolonRenderMode *mode) {
#if !defined(NDEBUG)
    const char *override = SDL_getenv("EIDOLON_BODY_RENDERER");
    if (override == NULL || override[0] == '\0') {
        return false;
    }
    if (SDL_strcmp(override, "sprite") == 0) {
        *mode = EIDOLON_RENDER_MODE_SPRITE;
        return true;
    }
    if (SDL_strcmp(override, "portrait") == 0) {
        *mode = EIDOLON_RENDER_MODE_PORTRAIT;
        return true;
    }
    if (SDL_strcmp(override, "model_3d") == 0) {
        *mode = EIDOLON_RENDER_MODE_MODEL_3D;
        return true;
    }
    eidolon_log_write("renderer", "ignoring invalid EIDOLON_BODY_RENDERER=%s", override);
#else
    (void)mode;
#endif
    return false;
...
static bool debug_render_mode_token(const char *token, size_t length, EidolonRenderMode *mode) {
    if (length == 6U && SDL_strncasecmp(token, "sprite", length) == 0) {
        *mode = EIDOLON_RENDER_MODE_SPRITE;
        return true;
    }
    if (length == 8U && SDL_strncasecmp(token, "portrait", length) == 0) {
        *mode = EIDOLON_RENDER_MODE_PORTRAIT;
        return true;
    }
    if (length == 8U && SDL_strncasecmp(token, "model_3d", length) == 0) {
        *mode = EIDOLON_RENDER_MODE_MODEL_3D;
        return true;
    }
    return false;
...
static size_t debug_body_sequence(EidolonRenderMode *modes, size_t capacity) {
    const char *sequence = SDL_getenv("EIDOLON_PRESENTATION_TEST_BODY_SEQUENCE");
    size_t count = 0U;
    if (sequence == NULL) {
        return 0U;
    }
    ...
    while (*cursor != '\0' && count < capacity) {
        ...
        if (end > start && !debug_render_mode_token(start, (size_t)(end - start), &modes[count])) {
            eidolon_log_write("renderer", "invalid body sequence token=%.*s", (int)(end - start),
                              start);
            return 0U;
        }
        if (end > start) {
...
static EidolonPresentation *create_sdl_legacy_presentation(bool snapshot_mode, bool start_hidden,
    ...
    SDL_WindowFlags flags = snapshot_mode
                                ? SDL_WINDOW_HIDDEN
...
bool eidolon_app_init(EidolonApp *app, EidolonAppMode mode) {
    ...
    app->authoring_mode = mode == EIDOLON_APP_AUTHORING;
    ...
    app->presentation_preference = EIDOLON_PRESENTATION_PREFERENCE_NATIVE;
    ...
    EidolonRenderMode system_render_mode = app->render_mode;
    EidolonRenderMode requested_render_mode = app->render_mode;
    if (configured_defaults_loaded &&
        eidolon_user_settings_is_overridden(&configured_defaults,
                                            EIDOLON_USER_SETTING_RENDER_MODE) &&
        configured_defaults.render_mode >= 0 &&
        configured_defaults.render_mode < (int)EIDOLON_RENDER_MODE_COUNT) {
        system_render_mode = (EidolonRenderMode)configured_defaults.render_mode;
        requested_render_mode = system_render_mode;
    }
    if (stored_settings_loaded &&
        ...
        requested_render_mode = (EidolonRenderMode)app->user_settings.render_mode;
    }
    ...
    EidolonPresentationPreference startup_preference = app->presentation_preference;
    const char *preference_source = "built-in";
    if (configured_defaults_loaded &&
        eidolon_user_settings_is_overridden(&configured_defaults,
                                            EIDOLON_USER_SETTING_PRESENTATION) &&
        configured_defaults.presentation_preference >= 0 &&
        configured_defaults.presentation_preference < (int)EIDOLON_PRESENTATION_PREFERENCE_COUNT) {
        startup_preference =
            (EidolonPresentationPreference)configured_defaults.presentation_preference;
        preference_source = "system";
    }
    if (stored_settings_loaded &&
        eidolon_user_settings_is_overridden(&app->user_settings,
                                            EIDOLON_USER_SETTING_PRESENTATION) &&
        app->user_settings.presentation_preference >= 0 &&
        app->user_settings.presentation_preference < (int)EIDOLON_PRESENTATION_PREFERENCE_COUNT) {
        startup_preference =
            (EidolonPresentationPreference)app->user_settings.presentation_preference;
    ...
    if (presentation_override != NULL) {
        if (SDL_strcmp(presentation_override, "win32_dcomp") == 0 ||
            SDL_strcmp(presentation_override, "native") == 0) {
            startup_preference = EIDOLON_PRESENTATION_PREFERENCE_NATIVE;
            preference_source = "environment";
        } else if (SDL_strcmp(presentation_override, "sdl_window_legacy") == 0 ||
                   SDL_strcmp(presentation_override, "sdl_legacy") == 0) {
            startup_preference = EIDOLON_PRESENTATION_PREFERENCE_SDL_LEGACY;
            preference_source = "environment";
        } else {
            eidolon_log_write("renderer", "ignoring invalid EIDOLON_PRESENTATION_BACKEND=%s",
                              presentation_override);
        }
    }
    if (app->snapshot_mode) {
        startup_preference = EIDOLON_PRESENTATION_PREFERENCE_SDL_LEGACY;
        preference_source = "snapshot";
    }

#if defined(_WIN32)
    const bool native_available = true;
    const char *native_backend_name = "win32_dcomp";
#else
    ...
    if (presentation_selection.fallback) {
        SDL_snprintf(presentation_fallback_reason, sizeof(presentation_fallback_reason),
                     "%s; body=%s",
                     eidolon_presentation_selection_reason_name(presentation_selection.reason),
                     eidolon_render_mode_name(requested_render_mode));
    }
#if defined(_WIN32)
    ...
    bool native_presentation_active = presentation_backend_is(app->presentation, "win32_dcomp");
    ...
    if (eidolon_presentation_get_environment(app->presentation, &initial_environment)) {
        ...
        app->applied_presentation_environment = initial_environment;
    ...
    eidolon_log_write("renderer",
                      "presentation preference=%s source=%s requested=%s active=%s graphics=%s "
                      "capabilities=0x%llx%s%s",
                      eidolon_presentation_preference_name(startup_preference), preference_source,
                      startup_preference == EIDOLON_PRESENTATION_PREFERENCE_NATIVE
                          ? native_backend_name
                          : "sdl_window_legacy",
    ...
    if (!activate_initial_body_renderer(app, requested_render_mode)) {
        eidolon_log_write("renderer", "body initialization failed: %s", SDL_GetError());
        return false;
    }
    if (app->render_mode == EIDOLON_RENDER_MODE_PORTRAIT) {
        ...
        eidolon_log_write("renderer", "body active mode=%s; inactive renderers skipped",
                          eidolon_render_mode_name(app->render_mode));
    }

    if (configured_defaults_loaded) {
        EidolonUserSettings startup_defaults = configured_defaults;
        startup_defaults.overrides &= ~(uint32_t)EIDOLON_USER_SETTING_RENDER_MODE;
        apply_settings_layer(app, &startup_defaults);
    }
    capture_runtime_settings(app, &app->system_settings);
    app->system_settings.render_mode = (int)system_render_mode;

    if (stored_settings_loaded) {
        EidolonUserSettings startup_user_settings = app->user_settings;
        startup_user_settings.overrides &= ~(uint32_t)EIDOLON_USER_SETTING_RENDER_MODE;
        apply_settings_layer(app, &startup_user_settings);
        eidolon_log_write("settings", "user overrides loaded fields=0x%x",
                          app->user_settings.overrides);
    }
    if (has_body_renderer_override && !eidolon_app_set_render_mode(app, body_renderer_override)) {
        eidolon_log_write("renderer", "body override unavailable requested=%s active=%s reason=%s",
                          eidolon_render_mode_name(body_renderer_override),
                          eidolon_render_mode_name(app->render_mode), SDL_GetError());
        SDL_ClearError();
    }
    ...
    if (!app->snapshot_mode) {
        ...
        app->user_settings_ready = !app->authoring_mode && app->user_settings_path[0] != '\0';
    }
    return true;
}
...
const char *eidolon_app_model_name(const EidolonApp *app) {
    ...
    case EIDOLON_RENDER_MODE_MODEL_3D:
        return eidolon_model_body_name(app->model);
    case EIDOLON_RENDER_MODE_COUNT:
        break;
    }
    return "Unknown";
...
bool eidolon_app_set_render_mode(EidolonApp *app, EidolonRenderMode mode) {
    ...
    if (!ensure_body_renderer(app, mode)) {
        return false;
    }
...
void eidolon_app_set_presentation_preference(EidolonApp *app,
    ...
    if (preference < 0 || preference >= EIDOLON_PRESENTATION_PREFERENCE_COUNT ||
        app->presentation_preference == preference) {
        return;
    }
    app->presentation_preference = preference;
    eidolon_log_write(
        "renderer", "presentation preference=%s active=%s applies=next_launch",
        eidolon_presentation_preference_name(preference),
        app->presentation != NULL ? eidolon_presentation_backend_name(app->presentation) : "none");
    mark_user_settings_dirty(app, EIDOLON_USER_SETTING_PRESENTATION);
}
...
bool eidolon_app_update_performance_fixture(EidolonApp *app, uint64_t now_ms) {
    ...
    if (app == NULL || app->render_mode != EIDOLON_RENDER_MODE_MODEL_3D || app->model == NULL ||
        ...
        return SDL_SetError("EPR performance fixture requires an active VRM body");
    }
    if (!eidolon_performance_fixture_update(&app->performance_fixture, &app->performance_runtime,
                                            now_ms)) {
        app->performance_runtime_ready = false;
        return SDL_SetError("EPR fixture stopped at control revision %llu",
                            (unsigned long long)app->performance_runtime.control.revision);
    }
    control = eidolon_epr_runtime_control(&app->performance_runtime);
    ...
    app->performance_control_attempted_revision = control->revision;
    if (!projected) {
        eidolon_log_write("performance",
                          "VRM projection rejected control revision=%llu; last valid retained: %s",
                          (unsigned long long)control->revision, SDL_GetError());
        SDL_ClearError();
    }
    return true;
}
...
static bool calibration_source_control(const EidolonApp *app, EidolonVrmCalibrationAnchorId anchor,
    ...
    const uint64_t whole_tick = (uint64_t)target - (uint64_t)target % 20U;
    for (uint64_t tick = 0U; tick <= whole_tick; tick += 20U) {
        if (!eidolon_performance_fixture_update(&fixture, &runtime, tick)) {
            return false;
        }
    }
    if ((uint64_t)target != whole_tick && !eidolon_epr_runtime_step(&runtime, target)) {
        return false;
    }
    const EidolonCanonicalControl *source = eidolon_epr_runtime_control(&runtime);
    if (source == NULL) {
        return false;
    }
    *control = *source;
...
bool eidolon_app_select_vrm_calibration_anchor(EidolonApp *app,
    ...
    EidolonCanonicalControl source;
    if (app == NULL || !app->vrm_calibration_ready) {
        return SDL_SetError("VRM calibration is not active");
    }
    if (anchor == app->vrm_calibration_session.selected_anchor &&
        app->vrm_calibration_session.source_control.valid) {
        return true;
    }
    if (app->vrm_calibration_session.dirty &&
        anchor != app->vrm_calibration_session.selected_anchor) {
        SDL_strlcpy(app->vrm_calibration_session.error,
                    "accept or revert the current anchor before switching",
                    sizeof(app->vrm_calibration_session.error));
        return false;
    }
    if (!calibration_source_control(app, anchor, &source) ||
        !eidolon_vrm_calibration_session_select(&app->vrm_calibration_session, anchor, &source) ||
        !eidolon_app_apply_vrm_calibration_draft(app)) {
        return SDL_SetError("could not select VRM calibration anchor '%s'",
                            eidolon_vrm_calibration_anchor_name(anchor));
    }
    return true;
}
...
bool eidolon_app_begin_vrm_calibration(EidolonApp *app, const char *path) {
    ...
    EidolonEprBodyProfile body;
    if (app == NULL || app->render_mode != EIDOLON_RENDER_MODE_MODEL_3D || app->model == NULL) {
        return SDL_SetError("VRM calibration requires an active 3D model");
    }
    if (!eidolon_model_vrm_measurements(app->model, &measurements)) {
        return SDL_SetError("VRM calibration could not read model measurements");
    }
    if (!eidolon_model_vrm_calibration(app->model, &calibration)) {
        return SDL_SetError("VRM calibration could not read the loaded sidecar state");
    }
    if (!eidolon_model_body_profile(app->model, &body)) {
        return SDL_SetError("VRM calibration could not read the EPR body profile");
    }
    if (!eidolon_vrm_calibration_session_init(&app->vrm_calibration_session, &measurements, &body,
                                              &calibration, path,
                                              eidolon_model_vrm_projection_revision(app->model))) {
        return false;
    }
    app->vrm_calibration_ready = true;
    if (!eidolon_app_select_vrm_calibration_anchor(app, EIDOLON_VRM_CALIBRATION_NEUTRAL)) {
        app->vrm_calibration_ready = false;
        return false;
    }
    eidolon_log_write("calibration",
                      "VRM calibration session ready body=%s fingerprint=%016llx path=%s",
                      eidolon_model_body_name(app->model),
                      (unsigned long long)measurements.anatomy_fingerprint, path);
    return true;
}

static bool touch_vrm_calibration(EidolonApp *app) {
    app->vrm_calibration_session.dirty = true;
    app->vrm_calibration_session.saved = false;
...
bool eidolon_app_set_vrm_calibration_euler(EidolonApp *app, bool head, size_t component,
    ...
    float *euler = head ? app->vrm_calibration_session.draft.head_euler
                        : app->vrm_calibration_session.draft.torso_euler;
    euler[component] = SDL_clamp(radians, -SDL_PI_F, SDL_PI_F);
...
bool eidolon_app_set_vrm_calibration_arm_component(EidolonApp *app, bool pole, size_t component,
                                                   float value) {
    if (app == NULL || !app->vrm_calibration_ready || component >= 3U || !isfinite(value)) {
        return false;
    }
    EidolonVrmCalibrationArm *arm =
        &app->vrm_calibration_session.draft.arms[EIDOLON_VRM_CALIBRATION_RIGHT];
    float *vector = pole ? arm->elbow_pole : arm->hand_target;
    vector[component] = SDL_clamp(value, -2.0F, 2.0F);
    return touch_vrm_calibration(app);
}
...
bool eidolon_app_set_vrm_calibration_arm_weight(EidolonApp *app, float weight) {
    if (app == NULL || !app->vrm_calibration_ready || !isfinite(weight)) {
        return false;
    }
...
bool eidolon_app_revert_vrm_calibration_anchor(EidolonApp *app) {
    ...
    eidolon_vrm_calibration_session_revert(&app->vrm_calibration_session);
    return eidolon_app_apply_vrm_calibration_draft(app);
}

bool eidolon_app_commit_vrm_calibration_anchor(EidolonApp *app) {
    return app != NULL && app->vrm_calibration_ready &&
           eidolon_vrm_calibration_session_commit(&app->vrm_calibration_session);
}

bool eidolon_app_save_vrm_calibration(EidolonApp *app) {
    if (app == NULL || !app->vrm_calibration_ready ||
        !eidolon_vrm_calibration_session_save(&app->vrm_calibration_session)) {
        return false;
    }
...
bool eidolon_app_reset_user_setting(EidolonApp *app, EidolonUserSettingField field) {
    ...
    const uint32_t previous_overrides = app->user_settings.overrides;
    app->user_settings.overrides &= ~(uint32_t)field;
    app->user_settings_applying = true;
    bool applied = true;
    switch (field) {
    case EIDOLON_USER_SETTING_RENDER_MODE:
        applied =
            eidolon_app_set_render_mode(app, (EidolonRenderMode)app->system_settings.render_mode);
        break;
    case EIDOLON_USER_SETTING_PRESENTATION:
        eidolon_app_set_presentation_preference(
            app, (EidolonPresentationPreference)app->system_settings.presentation_preference);
        break;
    case EIDOLON_USER_SETTING_DISPLAY_SCALE:
    ...
    if (!applied) {
        app->user_settings.overrides = previous_overrides;
        eidolon_log_write("settings", "override reset rejected field=0x%x reason=%s",
                          (unsigned int)field, SDL_GetError());
        return false;
    }
    schedule_user_settings_save(app);
...
static void apply_presentation_updates(EidolonApp *app) {
    ...
    SDL_FPoint body_center = current_body_global_center(app);
    const EidolonPresentationEnvironment previous_environment =
        app->applied_presentation_environment;
    const bool output_removed =
        app->presentation_environment_pending &&
        presentation_output_was_removed(app, &previous_environment, &app->presentation_environment);
    bool body_center_relocated =
        output_removed && map_removed_output_body_center(
                              &previous_environment, &app->presentation_environment, &body_center);
    const char *fallback_anchor = body_center_relocated ? "output-local" : "unchanged";
    if (output_removed && !body_center_relocated &&
        (app->presentation_environment.valid_fields & EIDOLON_PRESENTATION_ENV_USABLE_BOUNDS) !=
            0U &&
        app->presentation_environment.usable_bounds.width > 0.0F &&
        app->presentation_environment.usable_bounds.height > 0.0F) {
        body_center.x = app->presentation_environment.usable_bounds.x +
                        app->presentation_environment.usable_bounds.width * 0.5F;
        body_center.y = app->presentation_environment.usable_bounds.y +
                        app->presentation_environment.usable_bounds.height * 0.5F;
        body_center_relocated = true;
        fallback_anchor = "conservative";
    }
    if (app->presentation_environment_pending) {
        update_presentation_policy(app);
        const bool coordinate_scale_changed = update_display_metrics(app);
        if (body_center_relocated) {
            clamp_body_center_to_environment(app, &app->presentation_environment, &body_center);
        }
        if (coordinate_scale_changed || body_center_relocated) {
            restore_body_global_center(app, body_center);
        }
        app->applied_environment_revision = app->presentation_environment.revision;
        app->applied_presentation_environment = app->presentation_environment;
    ...
    app->native_drag_completed = true;
    if (output_removed) {
        const SDL_FPoint applied_center = current_body_global_center(app);
#if !defined(NDEBUG)
        app->presentation_output_fallback_applied = true;
#endif
        eidolon_log_write("presentation",
                          "output fallback removed=%u active=%u anchor=%s body=%.1f,%.1f "
                          "usable=%.0f,%.0f %.0fx%.0f",
                          previous_environment.active_output.value,
                          app->presentation_environment.active_output.value, fallback_anchor,
                          applied_center.x, applied_center.y,
                          app->presentation_environment.usable_bounds.x,
                          app->presentation_environment.usable_bounds.y,
                          app->presentation_environment.usable_bounds.width,
                          app->presentation_environment.usable_bounds.height);
    }
}
...
static void handle_presentation_event(EidolonApp *app, const EidolonPresentationEvent *event) {
    ...
    case EIDOLON_PRESENTATION_EVENT_GRAPHICS_RESET_REQUIRED:
        if (event->data.graphics.reset_kind == EIDOLON_PRESENTATION_GRAPHICS_RESET_TARGETS) {
            ...
            eidolon_log_write("renderer", "presentation targets invalidated; redraw requested");
        } else {
...
static bool routed_pointer_targets_current_body(const EidolonApp *app,
                                                const EidolonPresentationPointerEvent *pointer) {
    if (pointer->scene_revision != app->scene_snapshot.revision) {
        return false;
    }
    for (size_t index = 0U; index < app->scene_snapshot.layer_count; ++index) {
        const EidolonSceneLayerSnapshot *layer = &app->scene_snapshot.layers[index];
        if (layer->id.value == pointer->layer.value && layer->kind == EIDOLON_SCENE_LAYER_BODY &&
...
static const char *presentation_reset_name(EidolonPresentationGraphicsResetKind reset_kind) {
    switch (reset_kind) {
...
static void discard_recovery_presentation(EidolonApp *app) {
    ...
    if (app->renderer != NULL) {
        ...
        if (app->sprite != NULL) {
            (void)eidolon_sprite_set_renderer(app->sprite, NULL);
        }
    }
...
static bool activate_recovery_presentation(EidolonApp *app, EidolonPresentation *presentation,
    ...
    if (presentation_backend_is(presentation, "sdl_window_legacy")) {
        ...
        if (app->renderer == NULL ||
            ...
            (app->sprite != NULL && !eidolon_sprite_set_renderer(app->sprite, app->renderer)) ||
            (app->text_renderer != NULL &&
             !eidolon_text_renderer_set_renderer(app->text_renderer, app->renderer))) {
            return false;
        }
    }
    if (!ensure_body_renderer(app, app->render_mode)) {
        return false;
    }
    if (app->model != NULL && app->vrm_calibration_ready &&
        !eidolon_app_apply_vrm_calibration_draft(app)) {
        return false;
    }
    ...
    app->presentation_environment = environment;
    app->applied_presentation_environment = environment;
    app->presentation_environment_valid = true;
    app->presentation_environment_pending = false;
    app->presentation_move_completion_pending = false;
    app->presentation_resync_pending = false;
    ...
    if (!eidolon_draw_frame(app) || !eidolon_presentation_configure_host(app->presentation) ||
        (app->window != NULL && !presentation_test_hidden() && !SDL_ShowWindow(app->window))) {
        return false;
    }
    return true;
}
...
#if !defined(NDEBUG)
    const char *exit_after_recovery = SDL_getenv("EIDOLON_PRESENTATION_TEST_EXIT_AFTER_RECOVERY");
    if (exit_after_recovery != NULL && SDL_strcmp(exit_after_recovery, "1") == 0) {
        app->running = false;
    }
#else
    (void)app;
...
static bool recover_presentation(EidolonApp *app) {
    ...
    const EidolonPresentationGraphicsResetKind reset_kind = app->presentation_recovery_kind;
    ...
    if (!eidolon_presentation_get_geometry(app->presentation, &geometry) &&
        ...
        (app->presentation_environment.valid_fields & EIDOLON_PRESENTATION_ENV_HOST_GEOMETRY) !=
            0U) {
...
#if !defined(NDEBUG)
    const char *force_fallback_value = SDL_getenv("EIDOLON_DCOMP_TEST_FORCE_RECOVERY_FALLBACK");
    force_fallback = force_fallback_value != NULL && SDL_strcmp(force_fallback_value, "1") == 0;
#endif
    ...
    if (candidate != NULL &&
        ...
        app->native_drag_completed = true;
        eidolon_log_write(
            "renderer", "presentation recovered requested=win32_dcomp active=win32_dcomp reset=%s",
    ...
    SDL_ClearError();
#else
    SDL_strlcpy(native_failure, "win32_dcomp is unavailable", sizeof(native_failure));
#endif

    EidolonPresentation *fallback = create_sdl_legacy_presentation(
        false, true, SDL_max(1, geometry.width), SDL_max(1, geometry.height));
    if (fallback != NULL && activate_recovery_presentation(app, fallback, &geometry, body_center)) {
        app->presentation_recovery_pending = false;
        app->presentation_recovery_kind = EIDOLON_PRESENTATION_GRAPHICS_RESET_NONE;
        app->native_drag_completed = true;
        eidolon_log_write(
...
static void handle_app_event(EidolonApp *app, const EidolonAppEvent *event) {
    ...
    case EIDOLON_APP_EVENT_POINTER_MOTION:
        ...
        break;
    case EIDOLON_APP_EVENT_POINTER_WHEEL:
        if (point_in_model_rect(app, event->data.pointer.x, event->data.pointer.y)) {
            eidolon_app_adjust_model_scale(app, event->data.pointer.wheel_y);
        }
        break;
...
#if defined(_WIN32) && !defined(NDEBUG)
static uint64_t debug_frame_trigger(const char *name) {
    ...
    return end != value && *end == '\0' && parsed > 0U ? (uint64_t)parsed : 0U;
}
#endif
...
#if defined(_WIN32) && !defined(NDEBUG)
    const uint64_t dcomp_test_reset_after_frames =
        debug_frame_trigger("EIDOLON_DCOMP_TEST_RESET_AFTER_FRAMES");
    const uint64_t dcomp_test_output_removal_after_frames =
        debug_frame_trigger("EIDOLON_DCOMP_TEST_REMOVE_ACTIVE_OUTPUT_AFTER_FRAMES");
    const uint64_t presentation_test_exit_after_frames =
        debug_frame_trigger("EIDOLON_PRESENTATION_TEST_EXIT_AFTER_FRAMES");
    uint64_t presented_frame_count = 0U;
    bool dcomp_test_reset_injected = false;
    bool dcomp_test_output_removal_injected = false;
    EidolonRenderMode body_sequence[8];
    const size_t body_sequence_count =
        debug_body_sequence(body_sequence, SDL_arraysize(body_sequence));
    size_t body_sequence_index = 0U;
#endif
    while (app->running) {
        ...
        if (frame_clock.previous_frame_ns != 0U &&
            ...
            const double gap_ms =
                (double)(now_ns - frame_clock.previous_frame_ns) / (double)SDL_NS_PER_MS;
            ++pending_hitch_count;
            ...
            if (event_now_ms >= next_hitch_log_ms) {
                eidolon_log_write("renderer", "presentation hitches count=%llu max_gap_ms=%.2f",
                                  (unsigned long long)pending_hitch_count, pending_hitch_max_ms);
        ...
        if (app->render_mode == EIDOLON_RENDER_MODE_MODEL_3D && app->performance_runtime_ready) {
            if (!eidolon_app_update_performance_fixture(app, now_ms)) {
                eidolon_log_write(
                    "performance",
                    "EPR fixture stopped; preserving last valid VRM control revision=%llu",
                    (unsigned long long)app->performance_runtime.control.revision);
                app->performance_runtime_ready = false;
                SDL_ClearError();
            }
        ...
        const bool frame_drawn = eidolon_draw_frame(app);
#if defined(_WIN32) && !defined(NDEBUG)
        ++presented_frame_count;
        if (body_sequence_index < body_sequence_count &&
            presented_frame_count >= (uint64_t)(body_sequence_index + 1U) * 2U) {
            const EidolonState previous_state = app->state;
            const size_t previous_sessions =
                eidolon_session_registry_visible_count(&app->session_registry);
            ...
            const SDL_FPoint current_center = current_body_global_center(app);
            const bool state_preserved =
                frame_drawn && switched && app->state == previous_state &&
                eidolon_session_registry_visible_count(&app->session_registry) ==
                    previous_sessions &&
                app->presentation == previous_presentation &&
                SDL_strcmp(eidolon_presentation_backend_name(app->presentation),
                           previous_backend) == 0 &&
                SDL_fabsf(current_center.x - previous_center.x) <= 1.0F &&
                SDL_fabsf(current_center.y - previous_center.y) <= 1.0F;
            eidolon_log_write(
                "renderer",
                "live body switch requested=%s active=%s success=%s state_preserved=%s "
                "outgoing_frame=%s backend=%s center_delta=%.2f,%.2f sessions=%zu",
                eidolon_render_mode_name(requested), eidolon_render_mode_name(app->render_mode),
                switched ? "yes" : "no", state_preserved ? "yes" : "no",
                frame_drawn ? "valid" : "invalid",
                eidolon_presentation_backend_name(app->presentation),
                current_center.x - previous_center.x, current_center.y - previous_center.y,
                previous_sessions);
            ++body_sequence_index;
            if (!state_preserved) {
                app->running = false;
            } else if (body_sequence_index == body_sequence_count) {
                const char *exit_after_sequence =
                    SDL_getenv("EIDOLON_PRESENTATION_TEST_EXIT_AFTER_BODY_SEQUENCE");
                if (exit_after_sequence != NULL && SDL_strcmp(exit_after_sequence, "1") == 0) {
                    app->running = false;
                }
            }
        }
        if (presentation_test_exit_after_frames > 0U &&
            presented_frame_count >= presentation_test_exit_after_frames) {
            eidolon_log_write("renderer",
                              "presentation startup proof complete active=%s body=%s frame=%llu "
                              "session_registry=%s sources=%s",
                              eidolon_presentation_backend_name(app->presentation),
                              eidolon_render_mode_name(app->render_mode),
                              (unsigned long long)presented_frame_count,
                              app->session_registry.dialogue_configured ? "ready" : "unavailable",
                              app->conversation_sources != NULL ? "ready" : "legacy_only");
            app->running = false;
        }
        ...
        if (!dcomp_test_output_removal_injected && dcomp_test_reset_after_frames == 0U &&
            dcomp_test_output_removal_after_frames > 0U &&
            presented_frame_count >= dcomp_test_output_removal_after_frames &&
            presentation_backend_is(app->presentation, "win32_dcomp")) {
            dcomp_test_output_removal_injected = true;
            eidolon_log_write("renderer",
                              "injecting deterministic active-output removal frame=%llu",
                              (unsigned long long)presented_frame_count);
            if (!eidolon_win32_dcomp_test_inject_active_output_removal(app->presentation)) {
                eidolon_log_write("renderer", "active-output removal injection failed: %s",
                                  SDL_GetError());
                SDL_ClearError();
            }
        }
        const char *exit_after_output_removal =
            SDL_getenv("EIDOLON_PRESENTATION_TEST_EXIT_AFTER_OUTPUT_REMOVAL");
        if (app->presentation_output_fallback_applied) {
            eidolon_log_write("presentation",
                              "output fallback frame complete=%s scene_revision=%llu sessions=%zu "
                              "fallback_dialogue=%s revealed=%zu",
                              frame_drawn ? "yes" : "no",
                              (unsigned long long)app->scene_snapshot.revision,
                              eidolon_session_registry_visible_count(&app->session_registry),
                              eidolon_dialogue_is_active(&app->dialogue) ? "active" : "inactive",
                              app->dialogue.revealed);
            app->presentation_output_fallback_applied = false;
            if (exit_after_output_removal != NULL &&
                SDL_strcmp(exit_after_output_removal, "1") == 0) {
                app->running = false;
            }
        }
#else
...
void eidolon_app_destroy(EidolonApp *app) {
    ...
    eidolon_sprite_destroy(app->sprite);
    eidolon_presentation_destroy(app->presentation);
    SDL_Quit();
    SDL_zero(*app);
}
