...
static EidolonEprBehaviorUnit *add_behavior(EidolonBehaviorPlan *plan,
                                            EidolonEprBehaviorKind kind,
                                            EidolonEprOpaqueId cause, uint16_t priority,
                                            uint16_t urgency) {
    const EidolonEprOpaqueId id = eidolon_epr_behavior_id(kind, cause);
    EidolonEprBehaviorUnit *behavior = find_mutable(plan, id);
    if (behavior != NULL) {
        behavior->retired = false;
        behavior->terminal_reason = EIDOLON_EPR_TERMINAL_NONE;
...
static bool add_idle(EidolonBehaviorPlan *plan) {
    EidolonEprBehaviorUnit *idle =
        add_behavior(plan, EIDOLON_EPR_BEHAVIOR_IDLE, 1U, 10U, 0U);
    if (idle == NULL) {
        return false;
    }
    return add_claim(plan, idle, EIDOLON_EPR_RESOURCE_TORSO, EIDOLON_EPR_CLAIM_ADDITIVE,
                     0, EIDOLON_EPR_FOREVER, 1U, 1U) &&
           add_claim(plan, idle, EIDOLON_EPR_RESOURCE_HEAD, EIDOLON_EPR_CLAIM_ADDITIVE,
                     0, EIDOLON_EPR_FOREVER, 1U, 1U);
}
...
static bool add_posture(EidolonBehaviorPlan *plan, EidolonEprBehaviorKind kind,
    ...
    return add_claim(plan, posture, EIDOLON_EPR_RESOURCE_TORSO, EIDOLON_EPR_CLAIM_BASE,
                     start, EIDOLON_EPR_FOREVER, 0U, 2U) &&
           add_claim(plan, posture, EIDOLON_EPR_RESOURCE_HEAD, EIDOLON_EPR_CLAIM_BASE,
                     start, EIDOLON_EPR_FOREVER, 0U, 2U) &&
           add_claim(plan, posture, EIDOLON_EPR_RESOURCE_LEFT_ARM_CHAIN,
                     EIDOLON_EPR_CLAIM_BASE, start, EIDOLON_EPR_FOREVER, 0U, 2U) &&
           add_claim(plan, posture, EIDOLON_EPR_RESOURCE_RIGHT_ARM_CHAIN,
                     EIDOLON_EPR_CLAIM_BASE, start, EIDOLON_EPR_FOREVER, 0U, 2U);
}

static bool add_gaze(EidolonBehaviorPlan *plan, EidolonEprBehaviorKind kind,
                     EidolonEprTick start, uint16_t priority, uint16_t urgency) {
    ...
    return add_claim(plan, gaze, EIDOLON_EPR_RESOURCE_EYES, EIDOLON_EPR_CLAIM_BASE,
                     start, EIDOLON_EPR_FOREVER, 0U, 2U) &&
...
static bool add_gesture(EidolonBehaviorPlan *plan, const EidolonEprSemanticBeat *beat,
    ...
    EidolonEprBehaviorUnit *gesture =
        add_behavior(plan, EIDOLON_EPR_BEHAVIOR_GESTURE_CONTRAST_RIGHT, beat->id, 700U,
                     urgency);
    ...
    if (gesture == NULL ||
        ...
        !constrain(plan, gesture, EIDOLON_EPR_PHASE_PREPARATION, EIDOLON_EPR_PHASE_ONSET,
                   210, 210) ||
        !constrain(plan, gesture, EIDOLON_EPR_PHASE_ONSET, EIDOLON_EPR_PHASE_PEAK, 100,
                   100) ||
        !constrain(plan, gesture, EIDOLON_EPR_PHASE_PEAK, EIDOLON_EPR_PHASE_RECOVERY, 60,
                   60) ||
        !constrain(plan, gesture, EIDOLON_EPR_PHASE_RECOVERY,
                   EIDOLON_EPR_PHASE_COMPLETION, 440, 440)) {
        return false;
    }
...
static bool add_settle(EidolonBehaviorPlan *plan, EidolonEprOpaqueId cause,
                       EidolonEprTick start) {
    EidolonEprBehaviorUnit *settle =
        add_behavior(plan, EIDOLON_EPR_BEHAVIOR_SETTLE_RIGHT_ARM, cause, 900U, 1000U);
    if (settle == NULL ||
        !set_phase(plan, settle, EIDOLON_EPR_PHASE_INTERRUPT, start, true) ||
        !set_phase(plan, settle, EIDOLON_EPR_PHASE_SETTLE, start + 320, false) ||
        !constrain(plan, settle, EIDOLON_EPR_PHASE_INTERRUPT, EIDOLON_EPR_PHASE_SETTLE,
                   320, 320)) {
        return false;
    }
    return add_claim(plan, settle, EIDOLON_EPR_RESOURCE_RIGHT_ARM_CHAIN,
                     EIDOLON_EPR_CLAIM_OVERRIDE, start, start + 320, 0U, 4U);
}
...
static bool rebuild_claims(EidolonBehaviorPlan *plan, const EidolonPerformanceIntent *intent) {
    ...
    case EIDOLON_EPR_MODE_LISTENING:
        return add_posture(plan, EIDOLON_EPR_BEHAVIOR_POSTURE_ATTENTIVE,
                           intent->observed_tick, 300U, intent->urgency) &&
               add_gaze(plan, EIDOLON_EPR_BEHAVIOR_GAZE_ATTENTION,
                        intent->observed_tick, 400U, intent->urgency);
    case EIDOLON_EPR_MODE_THINKING:
        return add_posture(plan, EIDOLON_EPR_BEHAVIOR_POSTURE_THINKING,
                           intent->observed_tick, 350U, intent->urgency) &&
               add_gaze(plan, EIDOLON_EPR_BEHAVIOR_GAZE_ATTENTION,
                        intent->observed_tick, 400U, intent->urgency);
    case EIDOLON_EPR_MODE_RESPONDING:
        if (!add_posture(plan, EIDOLON_EPR_BEHAVIOR_POSTURE_RESPONDING,
                         intent->observed_tick, 450U, intent->urgency) ||
            !add_gaze(plan, EIDOLON_EPR_BEHAVIOR_GAZE_RESPONSE,
                      intent->observed_tick, 500U, intent->urgency)) {
            return false;
        }
        return contrast == NULL || add_gesture(plan, contrast, intent->urgency);
    case EIDOLON_EPR_MODE_INTERRUPTED:
        if (!add_posture(plan, EIDOLON_EPR_BEHAVIOR_POSTURE_GUARDED,
                         intent->observed_tick, 800U, 1000U) ||
            !add_gaze(plan, EIDOLON_EPR_BEHAVIOR_GAZE_INTERRUPTED,
                      intent->observed_tick, 900U, 1000U)) {
            return false;
        }
...
bool eidolon_epr_plan_apply(const EidolonBehaviorPlan *previous,
    ...
    if (!rebuild_claims(candidate, intent) || !eidolon_epr_temporal_validate(&candidate->temporal)) {
        return false;
    }
    return true;
}
